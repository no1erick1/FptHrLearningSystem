...
namespace FptHrLearningSystem.Controllers
{
    ...
    public class TraineeListController : Controller
    {
        ...
        public ActionResult Details(string id)
        {
            ...
            var user = db.trainees.Include(x => x.UserCourses).FirstOrDefault(m => m.Id == id);
            if (user == null)
            {
                return HttpNotFound();
            }
        ...
        {
            return db.trainees.Any(e => e.Id == id);
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(string id, Trainee user)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
